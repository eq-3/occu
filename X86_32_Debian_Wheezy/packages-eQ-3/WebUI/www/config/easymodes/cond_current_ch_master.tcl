...
proc set_htmlParams {iface address pps pps_descr special_input_id peer_type} {
  ...
  set MEASUREMENT_TYPE CURRENT
  set typeSelector j_current
  set selectBoxId currentSelectId
	...
	append HTML_PARAMS(separate_1) "<table class=\"ProfileTbl\">"
      ...
      append HTML_PARAMS(separate_1) "<tr><td colspan='2'>"
        append HTML_PARAMS(separate_1) "<select id=\"$selectBoxId\" onchange=\"setMode(this, '$chn')\">"
          append HTML_PARAMS(separate_1) "<option value='0'>\${currentOptionExpert}</option>"
          append HTML_PARAMS(separate_1) "<option value='1'>\${currentOptionHi}</option>"
          append HTML_PARAMS(separate_1) "<option value='2'>\${currentOptionLo}</option>"
          append HTML_PARAMS(separate_1) "<option value='3'>\${currentOptionMasterSlave}</option>"
  ...
  append HTML_PARAMS(separate_1) "<script type=\"text/javascript\">load_JSFunc('/config/easymodes/js/MeasurementSwitch.js');"
    append HTML_PARAMS(separate_1) "var current = {}; jQuery.extend(current, measurementSwitch);"
    append HTML_PARAMS(separate_1) "current.init('$MEASUREMENT_TYPE');"
    append HTML_PARAMS(separate_1) "current.showActiveProfile('$chn');"

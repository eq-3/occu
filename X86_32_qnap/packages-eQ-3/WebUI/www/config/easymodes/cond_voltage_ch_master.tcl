...
proc set_htmlParams {iface address pps pps_descr special_input_id peer_type} {
  ...
  set MEASUREMENT_TYPE VOLTAGE
  set typeSelector j_voltage
  set selectBoxId voltageSelectId
	...
	append HTML_PARAMS(separate_1) "<table class=\"ProfileTbl\">"
      ...
      append HTML_PARAMS(separate_1) "<tr><td colspan='2'>"
        append HTML_PARAMS(separate_1) "<select id=\"$selectBoxId\" onchange=\"setMode(this, '$chn')\">"
          append HTML_PARAMS(separate_1) "<option value='0'>\${voltageOptionExpert}</option>"
          append HTML_PARAMS(separate_1) "<option value='1'>\${voltageOptionHi}</option>"
          append HTML_PARAMS(separate_1) "<option value='2'>\${voltageOptionLo}</option>"
          append HTML_PARAMS(separate_1) "<option value='3'>\${voltageOptionMasterSlave}</option>"
  ...
  append HTML_PARAMS(separate_1) "<script type=\"text/javascript\">load_JSFunc('/config/easymodes/js/MeasurementSwitch.js');"
    append HTML_PARAMS(separate_1) "var voltage = {}; jQuery.extend(voltage, measurementSwitch);"
    append HTML_PARAMS(separate_1) "voltage.init('$MEASUREMENT_TYPE');"
    append HTML_PARAMS(separate_1) "voltage.showActiveProfile('$chn');"

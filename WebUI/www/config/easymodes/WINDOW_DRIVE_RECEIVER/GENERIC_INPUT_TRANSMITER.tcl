...
set PROFILE_2(LONG_JT_RAMPOFF)  3
...
set PROFILE_3(LONG_JT_RAMPON)  6
...
#1
  ...
  if {$parentType == "hmip-wgc"} {
    ...
    append HTML_PARAMS(separate_$prn) "[getTimeSelector WINDOW_ONTIME_FACTOR_DESCR ps PROFILE_$prn timeOnOffShort $prn $special_input_id SHORT_ON_TIME TIMEBASE_LONG]"
  } else {
  ...
  if {$longKeypressAvailable} {
    ...
    # ONDELAY
    append HTML_PARAMS(separate_$prn) "[getTimeSelector WINDOW_ONDELAY_TIME_FACTOR_DESCR ps PROFILE_$prn delay $prn $special_input_id LONG_ONDELAY_TIME TIMEBASE_LONG]"

    # ON_TIME
    if {$parentType == "hmip-wgc"} {
      # This is for the Garage Door Controller - we need a short ontime optionbox
      append HTML_PARAMS(separate_$prn) "[getTimeSelector WINDOW_ONTIME_FACTOR_DESCR ps PROFILE_$prn timeOnOffShort $prn $special_input_id LONG_ON_TIME TIMEBASE_LONG]"
    } else {
      # This is for all other switch actuators - here we have a long ontime optionbox
      append HTML_PARAMS(separate_$prn) "[getTimeSelector WINDOW_ONTIME_FACTOR_DESCR ps PROFILE_$prn timeOnOff $prn $special_input_id LONG_ON_TIME TIMEBASE_LONG]"
    }
  }

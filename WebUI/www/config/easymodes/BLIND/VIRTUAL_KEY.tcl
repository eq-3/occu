...
#5 short slat up - long drive up
  ...
  append HTML_PARAMS(separate_$prn) "<script type=\"text/javascript\">"
   ...
   append HTML_PARAMS(separate_$prn) "window.setTimeout(function() {document.getElementById('separate_${special_input_id}_$prn\_$pref').value = [expr $shortMaxTimeFirstDirUp + 0.01];},200);"
...
#6 short slat down - long drive down
  ...
  append HTML_PARAMS(separate_$prn) "<script type=\"text/javascript\">"
   # don`t use jQuery - the dirty flag will not be recognized
   append HTML_PARAMS(separate_$prn) "window.setTimeout(function() {document.getElementById('separate_${special_input_id}_$prn\_$pref').value = [expr $shortMaxTimeFirstDirDown + 0.01];},200);"
  append HTML_PARAMS(separate_$prn) "</script>"

...
#  die Texte der Platzhalter einlesen
  puts "<script type=\"text/javascript\">getLangInfo('$dev_descr_sender(TYPE)_3', '$dev_descr_receiver(TYPE)');</script>"
  puts "<script type=\"text/javascript\">getLangInfo_Special('HmIP_DEVICES.txt');</script>"
  puts "<script type=\"text/javascript\">jQuery('#dev_descr_sender_tmp').val('$dev_descr_sender(TYPE)_3');</script>"
...
#1
  ...
  append HTML_PARAMS(separate_$prn) "<td colspan =\"2\" style=\"padding-bottom:10px;\"><hr>\${description_fastRotation}</td>"
...
#3
  ...
  append HTML_PARAMS(separate_$prn) "<table class=\"ProfileTbl\">"
    ...
    if {$longKeypressAvailable} {
      append HTML_PARAMS(separate_$prn) "<td colspan =\"2\" style=\"padding-bottom:10px;\"><hr>\${description_slowRotation}</td>"

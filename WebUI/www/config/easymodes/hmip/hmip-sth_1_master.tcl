...
proc set_htmlParams {iface address pps pps_descr special_input_id peer_type} {
  ...
  append HTML_PARAMS(separate_1) "<table class=\"ProfileTbl\">"
    ...
    if { ! [catch {set tmp $ps($param)}]  } {
      ...
      append HTML_PARAMS(separate_1) "<script type=\"text/javascript\">"
        append HTML_PARAMS(separate_1) "jQuery(\"#separate_$CHANNEL\_$prn\").bind(\"blur\",function() {ProofAndSetValue(this.id, this.id, [_getMinValue $param], [_getMinValue $param], 1);isEcoLTComfort(this.name);});"
      append HTML_PARAMS(separate_1) "</script>"
    ...
    array_clear options
    set i 0
    for {set val [_getMinValue $param]} {$val <= [_getMinValue $param]} {set val [expr $val + 0.5]} {
      set options($i) "$val &#176;C"
      incr i;
    }

...
set PROFILE_3(SHORT_PROFILE_ACTION_TYPE) {2 1}
...
proc set_htmlParams {iface address pps pps_descr special_input_id peer_type} {
  ...
  if {$cur_profile == "3"} {
    if {$ps(SHORT_PROFILE_ACTION_TYPE) != 2} {
      # Set SHORT_PROFILE_ACTION_TYPE to 2 so we toggle with each switch command
      set value "{SHORT_PROFILE_ACTION_TYPE {int 2}}"
      set ps(SHORT_PROFILE_ACTION_TYPE) 2
      catch {puts "[xmlrpc $iface_url($iface) putParamset [list string $address] [list string $dev_descr_sender(ADDRESS)] [list struct $value]]"}
    }
  }

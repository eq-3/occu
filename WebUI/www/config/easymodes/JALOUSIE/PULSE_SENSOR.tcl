...
set PROFILES_MAP(1)  "\${blindOpenUp}"
set PROFILES_MAP(2)  "\${blindCloseDown}"
set PROFILES_MAP(3)  "\${blindOpenClose_UpDown}"
...
set PROFILE_1(SHORT_MAX_TIME_FIRST_DIR)  {25.5 0.1}
set PROFILE_1(SHORT_DRIVING_MODE)  0
set PROFILE_1(UI_DESCRIPTION)  "Bei Senden des Schaltbefehls f&auml;hrt die Jalousie/Markise/Rolllade f&uuml;r die eingestellte Zeit ganz hoch."
...
set PROFILE_2(UI_DESCRIPTION)  "Bei Senden des Schaltbefehls f&auml;hrt die Jalousie/Markise/Rolllade f&uuml;r die eingestellte Zeit ganz runter."
...
set PROFILE_3(UI_DESCRIPTION)    "Bei Senden des Schaltbefehls f&auml;hrt die Jalousie/Markise/Rolllade im Wechsel ganz hoch bzw. ganz runter (Togglefunktion)."
...
proc set_htmlParams {iface address pps pps_descr special_input_id peer_type} {
  ...
  if {$ps(SHORT_MAX_TIME_FIRST_DIR) == 0.1} {
    set url $iface_url($iface)
    set valSHORT_MAX_TIME_FIRST_DIR 25.5
    set param  "{SHORT_MAX_TIME_FIRST_DIR {$valSHORT_MAX_TIME_FIRST_DIR}}"
    puts "[xmlrpc $url putParamset [list string $receiver_address] [list string $sender_address] [list struct $param]]"
    set ps(SHORT_MAX_TIME_FIRST_DIR) $valSHORT_MAX_TIME_FIRST_DIR
  }

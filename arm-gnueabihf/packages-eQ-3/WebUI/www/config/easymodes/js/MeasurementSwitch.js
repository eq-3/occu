measurementSwitch = {
  ...
  _getActiveProfile: function(ch) {
    ...
    //var decisionValOk = (txDecisionAboveElem.val() == "200" && txDecisionBelowElem.val() == "0") ? true : false ;
...
isValValid = function (elem, chType) {
  var valLO=0, valHI=0;
  if (0 != getSelectedValue(chType)) {
    if ((elem.name.indexOf("THRESHOLD_HI_") != -1) || ((elem.name.indexOf("lowerVal") != -1)))  {
      valHI = parseFloat(jQuery("[name='COND_TX_THRESHOLD_HI_"+chType+"']").first().val());
      valLO = parseFloat(jQuery("[name='lowerVal_"+chType+"']").first().val());
    } else if ((elem.name.indexOf("THRESHOLD_LO_") != -1) || ((elem.name.indexOf("upperVal") != -1))) {
      valLO = parseFloat(jQuery("[name='COND_TX_THRESHOLD_LO_"+chType+"']").first().val());
...
correctValues = function(chType) {
  // chType = POWER, CURRENT, VOLTAGE or FREQUENCY
  var loElem = jQuery("[name='COND_TX_THRESHOLD_LO_"+chType+"']").first(),
    hiElem = jQuery("[name='COND_TX_THRESHOLD_HI_"+chType+"']").first(),
...
setVal = function(elem) {
  ...
  if (channelType) {
    ...
    if(!isValValid(elem, channelType)) {
  ...
  if (selMode == 1 || selMode == 2) {
    // Main value changed
    if (elem.name.indexOf("THRESHOLD_HI_") != -1) {
    ...
    } else if (elem.name.indexOf("THRESHOLD_LO_") != -1) {
      // LO Wert
